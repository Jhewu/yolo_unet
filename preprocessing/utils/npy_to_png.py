...
def ConvertNpyToPng(npy_path, output_dir):
    print("This directory exists", os.path.exists(npy_path))
    print("This is the directory", npy_path)
...
def Main(): 
    for mod in MODALITY:
        gt_folder = f"binarized_{mod}/masks"
        ...
        for split in gt_dir_list: 
            ...
            for image in input_dir_list: 
                img_path = os.path.join(input_dir, image)
                ConvertNpyToPng(img_path, output_dir)
